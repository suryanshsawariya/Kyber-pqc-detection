...
def server_gui(log_text):
    ...
    log_text.insert(tk.END, "🔵 Server is running and waiting for connection...\n", "info")

    client_socket, addr = server_socket.accept()
    secure_socket = context.wrap_socket(client_socket, server_side=True)
    log_text.insert(tk.END, f"🔵 Connection established with {addr}\n", "info")
    ...
    if data:
        ...
        log_text.insert(tk.END, "🟢 Confirmation sent to client\n", "success")
...
def client_gui(message, log_text):
    try:
        ...
        log_text.insert(tk.END, f"🟡 Sending message: {message}\n", "info")
        secure_socket.send(message.encode())
        data = secure_socket.recv(1024).decode()
        log_text.insert(tk.END, f"🟢 Received from server: {data}\n", "success")
        secure_socket.close()
...
def kyber_demo_gui(message, output_text):
    try:
        output_text.insert(tk.END, "🟠 Simulating Kyber keys...\n", "info")
        ...
        if shared_secret == recovered_shared_secret:
            output_text.insert(tk.END, "✅ Success! Shared secrets match.\n", "success")
        else:
            output_text.insert(tk.END, "❌ Error: Shared secrets do not match!\n", "error")
...
def qkd_demo_gui(output_text):
    output_text.insert(tk.END, "🟣 Simulating Quantum Key Distribution (QKD)...\n", "info")
    key_length = 128
...
def benchmark_function(func, output_text, iterations=10):
    ...
    output_text.insert(tk.END, f"⏱ Average execution time over {iterations} runs: {avg_time:.6f} seconds\n", "highlight")
    return avg_time
...
def kyber_benchmark_gui(output_text):
    try:
        ...
        output_text.insert(tk.END, f"⏱ Kyber Key Generation Time (avg): {avg_keygen_time:.6f} seconds\n", "highlight")
        output_text.insert(tk.END, f"⏱ Kyber Encryption Time (avg): {avg_encap_time:.6f} seconds\n", "highlight")
        output_text.insert(tk.END, f"⏱ Kyber Decryption Time (avg): {avg_decap_time:.6f} seconds\n", "highlight")
    except Exception as e:
        messagebox.showerror("Error", str(e))
...
def robustness_testing_gui(output_text):
    output_text.insert(tk.END, "🟡 Running Robustness Testing for Kyber PQC...\n", "info")
    ...
    try:
        ...
        output_text.insert(tk.END, "❌ Decryption failed as expected with corrupted ciphertext\n", "error")
    except Exception as e:
    ...
    try:
        ...
        output_text.insert(tk.END, "❌ Noise caused decryption mismatch\n", "error")
    except Exception as e:
        output_text.insert(tk.END, f"Error: {str(e)}\n", "error")
    ...
    try:
        ...
        output_text.insert(tk.END, "❌ Key mismatch detected, decryption failed\n", "error")
    except Exception as e:
        output_text.insert(tk.END, f"Error: {str(e)}\n", "error")
...
def analyze_security_gui(output_text):
    ...
    output_text.insert(tk.END, f"🔒 PQC Security: {pqc_strength}\n", "highlight")
    output_text.insert(tk.END, f"🔒 QKD Security: {qkd_security}\n", "highlight")

def analyze_performance_gui(output_text):
    output_text.insert(tk.END, "⚙ Analyzing performance...\n", "info")
    ...
    output_text.insert(tk.END, f"💻 Memory Usage: {memory_usage:.2f} MB\n", "info")
    output_text.insert(tk.END, f"💻 CPU Usage: {cpu_usage:.2f}%\n", "info")
...
def main():
    ...
    notebook.add(tab1, text="🔗 Server & Client Communication")
    notebook.add(tab2, text="🔐 Kyber PQC Demonstration")
    notebook.add(tab3, text="🔑 QKD Simulation")
    notebook.add(tab4, text="⚙ Benchmarking")
    notebook.add(tab5, text="🔍 Security & Performance Analysis")
    notebook.add(tab6, text="🛡 Robustness Testing")  # Adding Robustness Testing Tab
